...
class Trainer(object):
    def __init__(self,  weight_path, resume, gpu_id):
        ...
        self.scheduler = cosine_lr_scheduler.CosineDecayLR(self.optimizer,
                                                          ...
                                                          lr_init=cfg.TRAIN["LR_INIT"], lr_min=cfg.TRAIN["LR_END"], warmup=cfg.TRAIN["WARMUP_EPOCHS"]*len(self.train_dataloader))
        self.load_logging()
    ...
    def load_logging(self):
        ...
        self.testwriter = SummaryWriter('{}/{}'.format(self.work_dir, 'Test'))
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                self.trainwriter.add_scalar('Loss', mloss[3], len(self.train_dataloader) * epoch + i)
                self.trainwriter.add_scalar('Loss_GIOU', mloss[0], len(self.train_dataloader) * epoch + i)
                self.trainwriter.add_scalar('Loss_conf', mloss[1], len(self.train_dataloader) * epoch + i)
                self.trainwriter.add_scalar('Loss_cls', mloss[2], len(self.train_dataloader) * epoch + i)
            ...
            if epoch >= 1 and epoch % 10 == 0:
                ...
                with torch.no_grad():
                    APs, IOUs = Evaluator(self.yolov3).APs_voc()
                    for i in APs:
                    ...
                    print('mAP:%g'%(mAP))
                    self.testwriter.add_scalar('mAP', mAP, epoch)
